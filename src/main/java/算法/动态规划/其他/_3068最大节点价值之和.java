...
public class _3068最大节点价值之和 {
    /*
    n个节点无向树, 节点编号 0 ~ n-1
    给出nums数组, nums[i]表示节点i的价值
    给定edges数组, edges[i] = [u,v] 表示节点u与v相连

    给定一个正整数k,可以执行任意次操作:
    若节点u与v相连, 可以将u的值和v的值都异或上k

    求最大节点价值之和
     */

    /**
     推论: 异或操作可以发生在任意两个节点之间:<br>
     如果对一条路径上的所有边进行操作<br>
     ...
     所以执行操作后,异或了k的个数一定是偶数<br>
     <p>
     问题转变为:在nums中选择偶数个数,将他们异或上k,求nums的最大和<br>
     <br>
     状态机:<br>
     0:当前有偶数个数异或了k;  1:当前有奇数个数异或了k
     <p>
     对每个数字x独立考虑:<br>
