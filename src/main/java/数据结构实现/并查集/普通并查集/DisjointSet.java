...
public class DisjointSet {
    ...
    */
    ...
    public int find(int x) {
        ...
        if (x == fa[x]) {// 索引对应自身的节点为集合中的老大
            return x;
        }
        int f = find(fa[x]);// 跳转上级查找老大
        return fa[x] = f; //路径压缩,在查找中将老大的值赋值过来, 下次在该集合里查找时路径缩短
    }

    // union 是让两个集合“相交”，即选出新老大，x、y 是原老大索引
    /*例:
    0 3 相连 -> union(0,3) -> 3索引对应值改为0 (如果令较小值为老大)
